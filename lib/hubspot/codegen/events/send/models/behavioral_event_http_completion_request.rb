      ...
      class BehavioralEventHttpCompletionRequest
        ...
        def initialize(attributes = {})
          if (!attributes.is_a?(Hash))
            fail ArgumentError, "The input argument (attributes) must be a hash in `Hubspot::Events::Send::BehavioralEventHttpCompletionRequest` initialize method"
          end

          # check to see if the attribute exists and convert string to symbol for hash key
          attributes = attributes.each_with_object({}) { |(k, v), h|
            if (!self.class.attribute_map.key?(k.to_sym))
              fail ArgumentError, "`#{k}` is not a valid attribute in `Hubspot::Events::Send::BehavioralEventHttpCompletionRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
            end
            h[k.to_sym] = v
          }
        ...
        def hash
          [occurred_at, event_name, utk, uuid, email, properties, object_id].hash
        end
        ...
        def _deserialize(type, value)
          ...
          when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
            ...
            klass = Hubspot::Events::Send.const_get(type)
