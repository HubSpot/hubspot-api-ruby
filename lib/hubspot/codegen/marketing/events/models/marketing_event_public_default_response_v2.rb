      ...
      class MarketingEventPublicDefaultResponseV2
        ...
        def initialize(attributes = {})
          if (!attributes.is_a?(Hash))
            fail ArgumentError, "The input argument (attributes) must be a hash in `Hubspot::Marketing::Events::MarketingEventPublicDefaultResponseV2` initialize method"
          end

          # check to see if the attribute exists and convert string to symbol for hash key
          attributes = attributes.each_with_object({}) { |(k, v), h|
            if (!self.class.attribute_map.key?(k.to_sym))
              fail ArgumentError, "`#{k}` is not a valid attribute in `Hubspot::Marketing::Events::MarketingEventPublicDefaultResponseV2`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
            end
            h[k.to_sym] = v
          }
        ...
        def hash
          [event_organizer, event_url, app_info, event_type, event_completed, end_date_time, created_at, start_date_time, custom_properties, event_cancelled, event_description, event_name, object_id, updated_at].hash
        end
